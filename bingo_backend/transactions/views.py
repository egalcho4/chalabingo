# views.py - Updated with agent permissions and payment account display
...
# In transactions/views.py, replace the UserListView with this simplified version:
        ...
        try:
            ...
            if start_date_str and end_date_str:
                try:
                    ...
                    users = users.filter(date_joined__date__gte=start_date, date_joined__date__lte=end_date)
                except:
                    pass
            ...
            for user in users:
                user_data = {
                    ...
                    'user_type': 'player'  # Default
                }
                
                # Get wallet balance
                try:
                    wallet = Wallet.objects.get(user=user)
                    user_data['balance'] = float(wallet.balance)
                except Wallet.DoesNotExist:
                    user_data['balance'] = 0
                
                # Determine user type
                if hasattr(user, 'agent_id') and user.agent_id:
                    # Player assigned to an agent
                    pass
                ...
                # Add phone number if available (check if user model has phone_number field)
            ...
            print(f"Error in UserListView: {e}")
            return Response(
                {'error': str(e)}, 
...
class IsAgent(BasePermission):
    ...
    def has_permission(self, request, view):
        return request.user.is_authenticated and hasattr(request.user, 'agent_profile')

# In permissions.py or at top of views.py
class IsAgentOrAdmin(BasePermission):
    def has_permission(self, request, view):
        return request.user.is_authenticated and (
            request.user.is_staff or 
            request.user.is_superuser or 
            hasattr(request.user, 'agent_profile') or
            Agent.objects.filter(user=request.user).exists()
        )

class IsOwnerOrAgentOrAdmin(BasePermission):
    """Check if user owns the object, is agent, or is admin"""
    def has_object_permission(self, request, view, obj):
        # Admin can do anything
        if request.user.is_staff or request.user.is_superuser:
            return True
        
        # If object has user attribute, check ownership
        if hasattr(obj, 'user'):
            return obj.user == request.user
        
        # Check if user is agent and owns the related payment account
        if hasattr(request.user, 'agent_profile'):
            agent = request.user.agent_profile
            if hasattr(obj, 'payment_account'):
                return obj.payment_account.agent == agent
        
        return False

# Helper function to get agent from user
def get_user_agent(user):
    """Get agent for a user, return None if not an agent"""
    try:
...
class PaymentAccountViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        ...
        if self.action in ['list', 'retrieve', 'active_accounts', 'by_payment_method', 
                          'payment_methods', 'public', 'agent_accounts', 'player_agent_accounts']:
    ...
    def update(self, request, *args, **kwargs):
        ...
        if not (request.user.is_staff or request.user.is_superuser or 
                (hasattr(request.user, 'agent_profile') and instance.agent == request.user.agent_profile)):
            return Response(
    ...
    def toggle_active(self, request, pk=None):
        ...
        # Check if user is admin or owner of the payment account
        if not (request.user.is_staff or request.user.is_superuser or 
                (hasattr(request.user, 'agent_profile') and payment_account.agent == request.user.agent_profile)):
            return Response(
                {"detail": "You do not have permission to perform this action."},
                status=status.HTTP_403_FORBIDDEN
            )
    ...
    def active_accounts(self, request):
        ...
        agent_username = request.query_params.get('agent_username')  # New parameter
        
        try:
            current_user = request.user
            
            # If agent_username is provided, get that agent's accounts
            if agent_username:
                try:
                    agent_user = User.objects.get(username=agent_username)
                    agent = Agent.objects.get(user=agent_user)
        ...
        # Add agent info to each account
        serializer = self.get_serializer(queryset, many=True)
        data = serializer.data
        
        # Enhance data with agent information
        for i, account in enumerate(queryset):
            if account.agent:
                data[i]['agent_info'] = {
                    'username': account.agent.user.username,
                    'phone_number': account.agent.phone_number,
                    'commission_rate': float(account.agent.commission_rate)
                }
    ...
    def player_agent_accounts(self, request, player_username=None):
        ...
        if not player_user.is_authenticated or (hasattr(player_user, 'user_type') and player_user.user_type != "player"):
            return Response(
...
class AgentMyDepositsView(APIView):
    ...
    def get(self, request):
        try:
            ...
            # Get payment account info for each deposit
            data = []
            for deposit in deposits:
                deposit_data = {
                    ...
                    'reference': '',  # Add empty reference field
                    'notes': deposit.admin_notes,  # Changed from deposit.notes to deposit.admin_notes
                    'approved_by': deposit.processed_by.username if deposit.processed_by else None,  # Changed field name
                    'approved_at': deposit.updated_at if deposit.status == 'approved' else None,
                }
                
                # Add payment account info
                if deposit.payment_account:
                    deposit_data['payment_account'] = {
                        'id': deposit.payment_account.id,
                        'account_name': deposit.payment_account.account_name,
                        'account_number': deposit.payment_account.account_number,
                        'phone_number': deposit.payment_account.phone_number,
                        'bank_name': deposit.payment_account.bank_name,
                        'payment_method': deposit.payment_account.payment_method,
                        'payment_method_display': deposit.payment_account.get_payment_method_display(),
                        'min_amount': float(deposit.payment_account.min_amount),
                        'max_amount': float(deposit.payment_account.max_amount),
                        'is_active': deposit.payment_account.is_active,
                    }
                    
                    # Add agent info
                    if deposit.payment_account.agent:
                        deposit_data['agent'] = {
                            'id': deposit.payment_account.agent.id,
                            'username': deposit.payment_account.agent.user.username,
                            'phone_number': deposit.payment_account.agent.phone_number,
                            'commission_rate': float(deposit.payment_account.agent.commission_rate),
                        }
                
                data.append(deposit_data)
    ...
    def get(self, request):
        try:
            if request.user.is_staff or request.user.is_superuser:
                ...
                # Agent can see withdrawals for their payment accounts
                agent = Agent.objects.get(user=request.user)
            ...
            # Get payment account info for each withdrawal
            data = []
            for withdrawal in withdrawals:
                withdrawal_data = {
                    ...
                    'approved_by': withdrawal.approved_by.username if withdrawal.processed_by else None,
                    'approved_at': withdrawal.updated_at,
                }
                
                # Add payment account info
                if withdrawal.payment_account:
                    withdrawal_data['payment_account'] = {
                        'id': withdrawal.payment_account.id,
                        'account_name': withdrawal.payment_account.account_name,
                        'account_number': withdrawal.payment_account.account_number,
                        'phone_number': withdrawal.payment_account.phone_number,
                        'bank_name': withdrawal.payment_account.bank_name,
                        'payment_method': withdrawal.payment_account.payment_method,
                        'payment_method_display': withdrawal.payment_account.get_payment_method_display(),
                        'min_amount': float(withdrawal.payment_account.min_amount),
                        'max_amount': float(withdrawal.payment_account.max_amount),
                        'is_active': withdrawal.payment_account.is_active,
                    }
                    
                    # Add agent info
                    if withdrawal.payment_account.agent:
                        withdrawal_data['agent'] = {
                            'id': withdrawal.payment_account.agent.id,
                            'username': withdrawal.payment_account.agent.user.username,
                            'phone_number': withdrawal.payment_account.agent.phone_number,
                            'commission_rate': float(withdrawal.payment_account.agent.commission_rate),
                        }
                
                data.append(withdrawal_data)
...
class AgentDashboardView(APIView):
    ...
    permission_classes = [IsAuthenticated]  # Agent-specific permission
    ...
    def calculate_stats(self, start_date, end_date):
        ...
        for agent in agents:
            ...
            agents_data.append({
                ...
                'phone_number': agent.phone_number,
                'total_deposits': float(agent_deposits['total'] or 0),
                'total_withdraws': float(agent_withdrawals['total'] or 0),
...
class DepositViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # For agents, only show deposits for their payment accounts
        ...
        # For regular users, show only their deposits
        return Deposit.objects.filter(user=self.request.user).order_by('-created_at')
    
    def get_object(self):
        """
        Override to ensure agents can only access deposits for their payment accounts
        """
        ...
        if hasattr(self.request.user, 'agent_profile'):
            try:
                user_ag=User.objects.get(username=request.user)
                agent = Agent.objects.get(user=user_ag)
                # Verify the deposit belongs to the agent's payment account
                if not obj.payment_account or obj.payment_account.agent != agent:
                    raise PermissionDenied("You don't have permission to access this deposit")
            except Agent.DoesNotExist:
                raise PermissionDenied("Agent profile not found")
        
        return obj
    
    def get_permissions(self):
        if self.action in ['approve', 'reject']:
            # Allow both admin and agent to approve/reject deposits for their payment accounts
        ...
        try:
            ...
            if serializer.is_valid():
                ...
                Transaction.objects.create(
                    ...
                    status='pending',
                    description=f'Deposit via {deposit.payment_account if hasattr(deposit, "payment_account") else "Unknown"}'
                )
                
                # Notify admin about new deposit
                self.notify_admin(deposit)
            ...
            return Response(
                {'error': 'ሒሳብ መግፋት አልተቻለም። እባክዎ እንደገና ይሞክሩ።'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    
    def notify_admin(self, deposit):
        """Notify admin about new deposit request"""
        try:
            # You can implement email notification or other notification methods here
            # For now, just log it
            logger.info(f"New deposit created by {deposit.user.email}: {deposit.amount}")
        except Exception as e:
...
class DepositViewSet(viewsets.ModelViewSet):
    ...
    def approve(self, request, pk=None):
        """Approve a deposit (admin/agent can approve for their payment accounts)"""
        try:
            # Get the deposit - use standard get_object without permission check
            try:
            ...
            # Check permissions
            if not request.user.is_staff and not request.user.user_type =="agent":
                # Check if user is an agent and owns the payment account
                if hasattr(request.user, 'agent_profile'):
                    try:
                        agent = Agent.objects.get(user=request.user)
                        if not deposit.payment_account or deposit.payment_account.agent != agent:
                            return Response(
                    ...
                    return Response(
                        {'error': 'You do not have permission to approve deposits'},
                        status=status.HTTP_403_FORBIDDEN
                    )
            ...
            elif request.content_type.startswith('multipart/form-data') or request.content_type.startswith('application/x-www-form-urlencoded'):
                notes = request.POST.get('notes', '')
            else:
                # Try to parse as JSON anyway
                try:
                    import json
                    notes = json.loads(request.body.decode('utf-8')).get('notes', '')
                except:
                    notes = ''
            ...
            Transaction.objects.create(
                ...
                transaction_type='deposit',
                status='completed',
                description=f'Deposit approved. {notes}'
            )
            
            # Create notification
            self.create_approval_notification(deposit)
    ...
    def reject(self, request, pk=None):
        """Reject a deposit (admin/agent can reject for their payment accounts)"""
        try:
            # Get the deposit - use standard get_object without permission check
            try:
                deposit = Deposit.objects.get(pk=pk)
            ...
            if not request.user.is_staff and not request.user.user_type =="agent":
                ...
                if hasattr(request.user, 'agent_profile'):
                    ...
                    return Response(
                        {'error': 'You do not have permission to reject deposits'},
                        status=status.HTTP_403_FORBIDDEN
                    )
            ...
            Transaction.objects.filter(
                ...
                status='failed',
                description=f'Deposit rejected. {notes}'
            )
            
            # Create notification
            self.create_rejection_notification(deposit)
    ...
    def create_approval_notification(self, deposit):
        """Create notification for deposit approval"""
        try:
            # Implement notification logic here (email, push notification, etc.)
            logger.info(f"Deposit {deposit.id} approved by {deposit.approved_by.email}")
        except Exception as e:
            logger.error(f"Failed to create approval notification: {str(e)}")
    
    def create_rejection_notification(self, deposit):
        """Create notification for deposit rejection"""
        try:
            # Implement notification logic here
            logger.info(f"Deposit {deposit.id} rejected by {deposit.approved_by.email}")
        except Exception as e:
            logger.error(f"Failed to create rejection notification: {str(e)}")
...
class WithdrawRequestViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # For agents, only show withdrawals for their payment accounts
        if hasattr(self.request.user, 'agent_profile'):
            try:
                agent = Agent.objects.get(user=self.request.user)
        ...
        # For regular users, show only their withdrawals
    ...
    def perform_create(self, serializer):
        try:
            ...
            Transaction.objects.create(
                ...
                status='pending',
                description=f'Withdrawal request to {serializer.validated_data.get("account_number", "Unknown")}'
            )
            
            # Save withdraw request
            withdraw_request = serializer.save(user=user)
            
            # Notify admin about new withdrawal request
            self.notify_admin(withdraw_request)
            
        except Exception as e:
            logger.error(f"Error creating withdraw request: {str(e)}")
            raise
    
    def notify_admin(self, withdraw_request):
        """Notify admin about new withdrawal request"""
        try:
            logger.info(f"New withdrawal request by {withdraw_request.user.email}: {withdraw_request.amount}")
        except Exception as e:
            logger.error(f"Failed to notify admin: {str(e)}")
    ...
    @action(detail=True, methods=['post'])
    def approve(self, request, pk=None):
        """Approve a withdraw request (admin/agent can approve for their payment accounts)"""
        try:
            ...
            if not request.user.is_staff and not request.user.user_type =="agent":
                ...
                if hasattr(request.user, 'agent_profile'):
                    try:
                        agent = Agent.objects.get(user=request.user)
                        if not withdraw_request.payment_account or withdraw_request.payment_account.agent != agent:
                            return Response(
                    ...
                    return Response(
                        {'error': 'You do not have permission to approve withdrawals'},
                        status=status.HTTP_403_FORBIDDEN
                    )
            ...
            if withdraw_request.status != 'pending':
                return Response(
                    {'error': f'Withdrawal is already {withdraw_request.status}'},
                    status=status.HTTP_400_BAD_REQUEST
                )
            ...
            Transaction.objects.filter(
                ...
                status='completed',
                description=f'Withdrawal approved. {notes}'
            )
            
            # Create notification
            self.create_withdraw_approval_notification(withdraw_request)
            
            return Response({
                'message': '✅ Withdrawal approved successfully',
                'withdraw_request_id': withdraw_request.id,
                'amount': float(withdraw_request.amount),
                'user': withdraw_request.user.username,
                'status': 'approved'
            })
        
        except Exception as e:
            logger.error(f"Error approving withdraw request: {str(e)}")
            return Response(
                {'error': f'Failed to approve withdrawal: {str(e)}'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    
    @action(detail=True, methods=['post'])
    def reject(self, request, pk=None):
        """Reject a withdraw request (admin/agent can reject for their payment accounts)"""
        try:
            # Get the withdrawal request
            try:
                withdraw_request = WithdrawRequest.objects.get(pk=pk)
            ...
            if not request.user.is_staff and not request.user.user_type =="agent":
                ...
                if hasattr(request.user, 'agent_profile'):
                    ...
                    return Response(
                        {'error': 'You do not have permission to reject withdrawals'},
                        status=status.HTTP_403_FORBIDDEN
                    )
            ...
            Transaction.objects.filter(
                ...
                status='failed',
                description=f'Withdrawal rejected. {notes}'
            )
            
            # Create notification
            self.create_withdraw_rejection_notification(withdraw_request)
            ...
            logger.error(f"Error rejecting withdraw request: {str(e)}")
            return Response(
                {'error': f'Failed to reject withdrawal: {str(e)}'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    
    def create_withdraw_approval_notification(self, withdraw_request):
        """Create notification for withdrawal approval"""
        try:
            logger.info(f"Withdrawal request {withdraw_request.id} approved by {withdraw_request.approved_by.email}")
        except Exception as e:
            logger.error(f"Failed to create withdrawal approval notification: {str(e)}")
    
    def create_withdraw_rejection_notification(self, withdraw_request):
        """Create notification for withdrawal rejection"""
        try:
            logger.info(f"Withdrawal request {withdraw_request.id} rejected by {withdraw_request.approved_by.email}")
        except Exception as e:
            logger.error(f"Failed to create withdrawal rejection notification: {str(e)}")
    def create_withdraw_approval_notification(self, withdraw_request):
        """Create notification for withdrawal approval"""
        try:
            logger.info(f"Withdrawal request {withdraw_request.id} approved by {withdraw_request.approved_by.email}")
...
class TransactionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def my_transactions(self, request):
        """Get current user's transactions with agent info"""
        ...
        if start_date and end_date:
            try:
                ...
                transactions = transactions.filter(created_at__date__gte=start, created_at__date__lte=end)
            except ValueError:
                pass
        
        # Get data with agent info
        data = []
        for transaction in transactions:
            ...
            # Add related deposit or withdrawal info
            if transaction.transaction_type == 'deposit':
                try:
                    deposit = Deposit.objects.get(transaction_id=transaction.id)
                    transaction_data['deposit'] = {
                        'id': deposit.id,
                        'payment_account': deposit.payment_account.account_name if deposit.payment_account else None,
                        'proof_image': deposit.proof_image.url if deposit.proof_image else None,
                    }
                    
                    # Add agent info if payment account has agent
                    if deposit.payment_account and deposit.payment_account.agent:
                    ...
                    pass
            elif transaction.transaction_type == 'withdraw_request':
                try:
                    withdrawal = WithdrawRequest.objects.get(transaction_id=transaction.id)
                    ...
                    # Add agent info if payment account has agent
                    if withdrawal.payment_account and withdrawal.payment_account.agent:
                        transaction_data['agent'] = {
                            'username': withdrawal.payment_account.agent.user.username,
                            'phone_number': withdrawal.payment_account.agent.phone_number,
                        }
